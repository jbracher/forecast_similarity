...
Then a quantile-based approximation of the Cramer distance is given by

$$
\text{CD}(F, G) \approx \frac{1}{K(K + 1)} \times \sum_{i = 1}^{2K - 1} b_i(b_i + 1)(q_{i + 1} - q_i).
$$

Actually I think that the approximation for small $K$ is better if we use

$$
\text{CD}(F, G) \approx \frac{1}{(K + 1)^2} \times \sum_{i = 1}^{2K - 1} b_i^2(q_{i + 1} - q_i),
$$
which can be motivated much more directly, too (without the detour of the WIS approximation). It's just a bit further from the WIS-type approximation, but I don't see why that would be a problem.
...
grid_x <- seq(from = -10, to = 25, by = 0.1)
p_F <- pnorm(grid_x, mu_F, sigma_F)
p_G <- pnorm(grid_x, mu_G, sigma_G)
...
# plot:
plot(values_K, values_approx_cd, ylim = c(0, 5), xlab = "K", ylab = "CD",
     pch = 15, type = "b", log = "x")
