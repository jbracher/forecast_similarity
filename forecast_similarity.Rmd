...
$$
\text{CvMD}(F, G) \approx \frac{1}{K} \times \sum_{k = 1}^{2K - 1} b_k(b_k + 1)(q_{i + 1} - q_i).
$$
...
The approximation (1) is not limited to CDFs like the one of $H$ with just one step (i.e. $H$ can only take one value $y$). After all, wen never make use of the restriction that $H$ has just one step. Indeed, we can apply it to approximate the Cramer von Mises distance of any pair of distributions $F$ and $G$:
$$
...
Here is an R function to compute this approximation of the CvMD:
...
approx_cvmd <- function(q_F, q_G){
  ...
  diffs <- c(diff(q), 0) # zero necessary for indexing below, but we could put 
  # anything (gets multiplied w zero)
  
  step <- diff(p)[1] # this is 1/K
  cvm <- sum(diffs*step*abs(b)*(abs(b) + 1))/length(p) # using mean does not work 
  # as we then divide by 2K... not entirely sure
  
  return(mean(cvm))
}
```

What is nice about this measure is that similarly to the WIS it can be decomposed into various components. We could for instance define decompositions according to

* whether $F(X)$ or $G(x)$ is larger to spot if certain models systematically predict larger values.
* whether the quantiles of $F$ or $G$ are more dispersed (not yet totally clear how to do that, but pretty sure it works).

These decompositions would also remain interpretable when aggregating across many forecasts. Also the measure preserves some desirable properties of the CvMD:
...
* It is symmetric (which is not the case for e.g. the Kullback Leibler divergence).
...
for(i in seq_along(values_K)){
  p_temp <- (1:(values_K[i] - 1))/values_K[i]
  q_F_temp <- qnorm(p_temp, mu_F, sigma_F)
  q_G_temp <- qnorm(p_temp, mu_G, sigma_G)
  values_approx_cvmd[i] <- approx_cvmd(q_F = q_F_temp, q_G = q_G_temp)
}

plot(values_K, values_approx_cvmd, ylim = c(0, 1), xlab = "K", ylab = "CvMD", pch = 15, type = "b")
abline(h = exact_cvmd, lty = 2)
legend("topright", c("approximation", "result from numerical integration"), pch = c(15, NA), lty = c(1, 2))
```